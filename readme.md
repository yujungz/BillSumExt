...
支持的站点：**ai、csp、pinova、wzg、qn、digitalcloud、wshk**
...
如果之前部署过旧版本（依赖外部 `test-mysql8` 容器），需先清理：
...
# 清理旧的外部网络（如存在）
docker network rm BillSumExt_net 2>/dev/null

# 删除旧 app_data 卷（会丢失旧配置，需重新配置）
docker volume rm BillSumExt_app_data 2>/dev/null
```

### 3.2 构建并启动所有服务

**项目已完全自包含**，不再依赖外部 MySQL 容器。docker-compose 会自动创建 MySQL 和应用两个容器。

```bash
docker-compose up -d --build
```

> MySQL 容器首次启动时会自动创建 `sum_all` 系统数据库并初始化表结构和数据。
...
1. 进入 **参数配置** 页面
2. 本地数据库连接已预设（主机 `BillSumExt-mysql`，端口 `3306`）
3. 切换到各站点 Tab 页，配置 SSH 连接信息和远程数据库参数
4. 点击 **「保存配置」**
5. 可在 **数据传输** 页面开始导入数据
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| 容器名称 | BillSumExt-mysql | MySQL Docker 容器名 |
...
检查 MySQL 容器是否在运行：`docker ps | grep BillSumExt-mysql`。确认应用容器和 MySQL 容器在同一 Docker 网络中：`docker network inspect BillSumExt_net`。

**Q: 远程导出失败？**

1. 确认 SSH 密钥文件路径正确，且已挂载到应用容器内
2. 确认远程服务器上的 Docker 容器名称和密码正确
3. 查看日志：`docker logs BillSumExt-app`
